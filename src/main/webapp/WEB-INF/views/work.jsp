...
function drop(ev) {
    ...
    $.each($("#active img"), function( index, value ) {
    	if(data.substr(data.indexOf("resources"), data.length) == $(value).attr("src")) { 
    		...
    		addItems.push({"class": $(value).attr("class"), "src":$(value).attr("src"), "index":$(".big1 div").index($(ev.target))});
    	}
    });
...
//     	console.log(data.substr(data.indexOf("resources"), data.length), addItems[i].src);
		if(data.substr(data.indexOf("resources"), data.length) == addItems[i].src){
...
//     	if(data.substr(data.indexOf("resources"), data.length) == $(addItems.eq()).attr("src")) { 
//     		console.log( index, $(value).attr("class"), $(value).attr("src") );
//     		$(value).
//     		addItems.;
//     	}
